...
// Copyright (c) 2011-2014 Platinum Developers.
...
uint256 hashGenesisBlock("0xbf9a867c0b2717026e2fe9a5f26fad5a05e4a21f36d9a224122fd84909e3072e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Platinum: starting difficulty is 1 / 2^12
...
static const int64 nTargetTimespan =  20 * 60; // Platinum: 20 Minute difficulty retarget
static const int64 nTargetSpacing = 30; // Platinum: 30 Second Bock time
...
// Platinum difficulty adjustment protocol switch
   ...
   int64 nInterval = fInflationFixProtocol? (nTargetTimespanCurrent / nTargetSpacing) : (nTargetTimespanCurrent / (nTargetSpacing / 2));
    ...
    // Platinum: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Platinum miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Platinum", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Platinum Cryptocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391006204;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1429777;

        if (fTestNet)
        {
            block.nTime    = 1391006204;             //test net not supported
            block.nNonce   = 0;
        }
		...
		assert(block.hashMerkleRoot == uint256("0xea20529f876da02a6074a4873c140f6675d7a096c60c30a1f70ebe6e6c0b696a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Platinum: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Platinumd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
